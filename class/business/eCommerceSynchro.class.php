    ...
    public function synchCategory($toNb=0)
    {
        ...
        try {
            ...
            if ($importRootExists)
            {
                ...
                {
                    $this->error=$this->langs->trans('ECommerceErrorsynchCategory').' 1. (Code FailToGetDetailsOfRecord)';
                    $this->errors[] = $this->error;
                }
            }
            else
...
//                            }
//                        }
                        // soumer : you may added this $this->eCommerceSite->fk_anonymous_thirdparty 
                        if ($result == 0 )
                        {
                            $dBSociete->name = $societeArray['name'];
                            //$dBSociete->ref_ext = $this->eCommerceSite->name.'-'.$societeArray['remote_id'];      // No need of ref_ext, we will search if already exists on name
...
//                            }
            ...
            {
                $this->error=$this->langs->trans('ECommerceErrorsynchSociete').' 2. (Code FailToGetDetailsOfRecord)';
                $this->errors[] = $this->error;
            }
        } catch (Exception $e) {
    ...
    public function synchProduct($toNb=0)
    {
        ...
        try {
			...
			*/
            ...
            {
                $this->error=$this->langs->trans('ECommerceErrorsynchProduct').' 3. (Code FailToGetDetailsOfRecord)';
                $this->errors[] = $this->error;
            }
        } catch (Exception $e) {
...
//                                    $dBSociete->town = $sCompany['town'];
            ...
            {
                $this->error=$this->langs->trans('ECommerceErrorsynchCommande').' 4. (Code FailToGetDetailsOfRecord)';
                $this->errors[] = $this->error;
            }
        } catch (Exception $e) {
...
//                $productsTypesOk = array('simple', 'virtual', 'downloadable');
            ...
            {
                $this->error=$this->langs->trans('ECommerceErrorsynchFacture').' 5. (Code FailToGetDetailsOfRecord)';
                $this->errors[] = $this->error;
            }
        } catch (Exception $e) {
